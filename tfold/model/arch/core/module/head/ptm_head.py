...
def compute_ptmscore(logits, max_bin=31, no_bins=64, residue_weights=None, asym_id=None, inter_chain_contact=None):
    ...
    if asym_id is not None:
        ...
        if inter_chain_contact is not None:
            pair_mask *= inter_chain_contact
...
class TMScoreHead(nn.Module):
    ...
    def forward(self, z, asym_id=None, inter_chain_contact=None):
        """
        Args:
            z: [N, L, L, c_z], pair features
            asym_id: (optional) the asymmetric unit ID (chain ID) of size L (multimer only)
            inter_chain_contact: (optional) [N, L, L, 1], predict inter_chain_contact predicted by DistogramHead
        Returns:
        ...
        if asym_id is not None:
            ...
            iptm = compute_ptmscore(logits[0], asym_id=asym_id)
            tmsc_dict['iptm'] = iptm
            tmsc_dict['ranking_confidence'] = 0.8 * iptm + 0.2 * ptm

            # calculate actifptm proposed by ColabFold team in https://arxiv.org/pdf/2412.15970
            if inter_chain_contact is not None:
                actifptm = compute_ptmscore(logits[0], asym_id=asym_id, inter_chain_contact=inter_chain_contact)
                tmsc_dict['actifptm'] = actifptm
